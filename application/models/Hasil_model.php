...
class Hasil_model extends CI_Model
{
    ...
    {
        //$query = "SELECT `usr`.`nama`, `usr`.`id`, `prfl`.`id_kelas` FROM users usr JOIN profile_siswa prfl ON `prfl`.`id_kelas`= $id_kelas AND `usr`.`id` = `prfl`.`id_user`";
        $query = "SELECT `usr`.`nama`, `usr`.`id`, `prfl`.`id_kelas`, `sl`.`id_mata_pelajaran` FROM users usr JOIN profile_siswa prfl ON `usr`.`id` = `prfl`.`id_user` JOIN soal sl ON `prfl`.`id_kelas` = `sl`.`id_kelas` WHERE `sl`.`id_kelas` = $id_kelas AND `sl`.`id_mata_pelajaran` = $id_mapel GROUP BY `usr`.`id`";
        return $this->db->query($query)->result_array();
    }
    ...
    {
        $query = " SELECT * FROM jawaban WHERE `id_user` = $id_user AND `id_mata_pelajaran` = $id_mapel AND `id_kelas` = $id_kelas ";
        return $this->db->query($query)->result_array();
    }
}
